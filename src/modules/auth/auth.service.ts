...
export class AuthService {
  ...
  async signup(userData): Promise<UserDocument | string> {
    try {
  ...
  async login(userData: LoginUserDto, user: UserDocument) {
    try {
      ...
      if (
        ...
        const timeUntilUnlock = new Date(user.lockUntil.getTime());
        return {
          message: `Account locked. Please try again after ${timeUntilUnlock.toLocaleString()}`,
          error: 'Unauthorized',
          statusCode: 401,
        };
      }
      ...
      if (matched) {
        ...
        const payload = { email: userData.email, user_id: user._id };
        return {
